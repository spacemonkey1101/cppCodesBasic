...
public:
    ...
    ~Product()
    {
        ...
        // inside the destructor we must remove all dynamically allocated member functions
        if (name != NULL)
        {
            delete name; // freed up memory; cleared whatever the pointer was pointing to
            name = NULL; // fixed the dangling pointer
        }
    }
};
...
int main()
{
    ...
    camera_old.setName("Kotak old gen");
